...
CREATE TABLE dossier(
   ...
   id_statut_dossier INT REFERENCES statut_dossier(id_statut_dossier),
   id_ville INT NOT NULL REFERENCES ville(id_ville)
);
...
CREATE TABLE suivi_dossier(
   ...
   id_statut_couleur INT REFERENCES statut_couleur(id_statut_couleur),
   commentaire TEXT,
   date_creation TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
   date_modification TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
   id_utilisateur INT REFERENCES utilisateur(id_utilisateur),
...
CREATE TABLE audit(
   ...
   entite_type VARCHAR(30) NOT NULL
      CHECK(entite_type IN('dossier','notification_occupation','avertissement',
                            ...
                            'geometrie','signalement','descente_terrain')),
...
CREATE TABLE photo(
   ...
   entite_type VARCHAR(30) NOT NULL
      CHECK(entite_type IN('notification','suivi','avertissement','signalement','descente_terrain')),
...
CREATE TABLE geometrie(
   ...
   entite_type VARCHAR(30) NOT NULL
      CHECK(entite_type IN('parcelle','titre_foncier','dossier_parcelle','signalement','descente_terrain')),
   entite_id UUID NOT NULL,
...
CREATE TABLE signalement(
   ...
   id_ville INT REFERENCES ville(id_ville),
   id_titre_foncier UUID REFERENCES titre_foncier(id_titre_foncier),
   id_parcelle UUID REFERENCES parcelle(id_parcelle),
